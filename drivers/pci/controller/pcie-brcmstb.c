...
static int brcm_pcie_post_setup_bcm2712(struct brcm_pcie *pcie)
{
	...
	if (!of_property_read_u32(pcie->np, "brcm,fifo-qos-map", &qos_map)) {
		/*
		...
		qos_map &= 0x0000ffff;
		for (i = 0; i < 8; i++)
			writel(qos_map, pcie->base + PCIE_MISC_TC_QUEUE_TO_QOS_MAP(i));

	} else if (!of_property_read_u32(pcie->np, "brcm,vdm-qos-map", &qos_map)) {
		...
		writel(qos_map, pcie->base + PCIE_MISC_VDM_PRIORITY_TO_QOS_MAP_LO);
		writel(qos_map, pcie->base + PCIE_MISC_VDM_PRIORITY_TO_QOS_MAP_HI);
