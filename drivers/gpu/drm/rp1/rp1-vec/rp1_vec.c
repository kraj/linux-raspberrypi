...
/*
 * Initial/default TV standard. If unspecified it will be NTSC (which defaults
 * to NTSC-M at 60Hz but can nevertheless still display PAL for 625/50Hz modes).
 * This module parameter, which is not mutable at run-time, may be overridden
 * from the command-line before initialising the DRM "tv_mode" property.
 * We add the non-standard value "Mono", which supports Systems M, BDGHIK, A, E
 * without colour. There is no "PAL60"; instead, use "PAL" with a 525/60Hz mode.
 */

static char *rp1vec_tv_norm_str;
module_param_named(tv_norm, rp1vec_tv_norm_str, charp, 0400);
MODULE_PARM_DESC(tv_norm, "Default TV norm.\n"
		 "\t\tSupported: NTSC, NTSC-443, NTSC-J, PAL, PAL-M, PAL-N, Mono.\n"
		 "\t\tDefault: NTSC (which also supports PAL for 625-line modes)\n");

static int rp1vec_parse_tv_norm(void)
{
	int i = DRM_MODE_TV_MODE_NTSC;

	if (rp1vec_tv_norm_str && *rp1vec_tv_norm_str) {
		i = drm_get_tv_mode_from_name(rp1vec_tv_norm_str,
					      strlen(rp1vec_tv_norm_str));
		if (i < 0)
			i = (strncmp(rp1vec_tv_norm_str, "Mono", 4) == 0) ?
				RP1VEC_TV_MODE_MONOCHROME : DRM_MODE_TV_MODE_NTSC;
	}
