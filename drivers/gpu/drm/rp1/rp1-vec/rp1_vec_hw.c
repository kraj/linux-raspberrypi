		     ...
		     int tvstd)
{
	 ...
	 * is wider than the horizontal active region, to hide FIR/CIC artifacts
	 * at scanline boundaries (e.g. wmax = 728, w = 720, active ~= 704).
	 */
	...
	usr_2mid = ((2 * (mode->htotal - mode->hsync_start) + w) * 2 * (hwm->timing_regs[1] >> 16)) /
		(rate * mode->htotal);
	hpad_r = max(0, (wmax - w + ref_2mid - usr_2mid) >> 1);
	hpad_r = min(hpad_r, wmax - w);
	h = mode->vdisplay >> (hwm->interlaced || vec->fake_31khz);
	h = min(h, 0 + hwm->max_rows_per_field);
	vpad_b = max(0, ((mode->vsync_start - hwm->ref_vfp) >> (hwm->interlaced || vec->fake_31khz)) - h);
	vpad_b = min(vpad_b, hwm->max_rows_per_field - h);
